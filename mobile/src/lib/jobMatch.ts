// Ported 1:1 from frontend/src/lib/jobMatch.js — client-side match-count + the
// salary/posted-ago formatters shared by the Jobs list and Job detail screens.
...
export function parseElp(str: unknown): number | null {
...
export function rtwMatchesRegion(country: string, region: string): boolean {
  const c = country.toLowerCase();
  if (region === 'EU') {
    return (
      /\beu\b|european union|europe/.test(c) ||
...
export function computeMatchCount(job: AnyRec, profile: AnyRec, totals: AnyRec) {
  const normType = (t: string) => (t === 'ATP' || t === 'ATPL' ? ['ATP', 'ATPL'] : [t]);
  const normAuth = (a: string) => (['CAA', 'CAA_UK', 'CAA-UK'].includes(a) ? ['CAA', 'CAA_UK', 'CAA-UK'] : [a]);

  const certTypes = [...new Set((profile.certificates || []).filter((c: AnyRec) => c.type !== 'ELP').flatMap((c: AnyRec) => normType(c.type)))];
  const certAuthorities = [...new Set((profile.certificates || []).filter((c: AnyRec) => c.type !== 'ELP').flatMap((c: AnyRec) => normAuth(c.issuingAuthority)))];
  const ratingTypes = (profile.ratings || []).map((r: AnyRec) => r.aircraftType.toUpperCase());

  const bestMedical = (profile.medicals || []).reduce((best: string | null, m: AnyRec) => {
    const rank: Record<string, number> = { CLASS_1: 3, CLASS_2: 2, CLASS_3: 1 };
  ...
  const req = (label: string, reqValue: string, icon: string, isMatch: boolean, pilotValue: string | null = null) =>
    requirements.push({ label, reqValue, icon, matched: isMatch, pilotValue });

  if (job.reqAuthorities?.length) {
    const isMatch = job.reqAuthorities.some((a: string) => certAuthorities.includes(a));
    const pilotAuth = (certAuthorities as string[]).find((a) => job.reqAuthorities.includes(a)) ?? null;
    req('Authority', job.reqAuthorities.join(', '), 'Building2', isMatch, pilotAuth);
  }
  if (job.reqCertificates?.length) {
    const isMatch = job.reqCertificates.some((c: string) => certTypes.includes(c));
    const pilotCert = (certTypes as string[]).find((c) => job.reqCertificates.includes(c)) ?? null;
    req('Certificate', job.reqCertificates.join(', '), 'FileText', isMatch, pilotCert);
  }
  if (job.reqMinTotalHours != null) {
    req('Total Hours', `${job.reqMinTotalHours.toLocaleString()} hrs min`, 'Clock', totalTime >= job.reqMinTotalHours, `${totalTime.toLocaleString()} hrs`);
  }
  if (job.reqMinPicHours != null) {
    req('PIC Hours', `${job.reqMinPicHours.toLocaleString()} hrs min`, 'Target', picTime >= job.reqMinPicHours, `${picTime.toLocaleString()} hrs`);
  }
  if (job.reqMedicalClass != null) {
    const isMatch = qualifiedMedicals.includes(job.reqMedicalClass);
    req('Medical Class', job.reqMedicalClass.replace('CLASS_', 'Class '), 'Shield', isMatch, bestMedical ? bestMedical.replace('CLASS_', 'Class ') : null);
  }
  if (job.reqAircraftTypes?.length) {
    const isMatch = job.reqAircraftTypes.some((a: string) => ratingTypes.includes(a.toUpperCase()));
    const pilotType = (ratingTypes as string[]).find((r) => job.reqAircraftTypes.some((a: string) => a.toUpperCase() === r)) ?? null;
  ...
  if (job.reqEducation) {
    ...
    req('Education', EDU_LABEL[job.reqEducation] || job.reqEducation, 'GraduationCap', pilotRank >= reqRank, profile.education ? EDU_LABEL[profile.education] : null);
  }
  if (job.reqWorkAuthorization) {
    const rtw = profile.rightToWork || [];
    const isMatch = job.reqWorkAuthorization === 'required' ? rtw.length > 0 : rtw.some((r: AnyRec) => rtwMatchesRegion(r.country, job.reqWorkAuthorization));
    const matchingRtw = rtw.find((r: AnyRec) => rtwMatchesRegion(r.country, job.reqWorkAuthorization));
    req('Work Auth', WA_LABEL[job.reqWorkAuthorization] || job.reqWorkAuthorization, 'Globe', isMatch, matchingRtw ? matchingRtw.country : rtw.length > 0 && job.reqWorkAuthorization === 'required' ? rtw[0].country : null);
  }
  if (job.reqEnglishLevel != null) {
    const elpCerts = (profile.certificates || []).filter((c: AnyRec) => c.type === 'ELP');
    const maxLevel = elpCerts.reduce((m: number, c: AnyRec) => {
      const lvl = parseElp(c.englishLevel);
      return lvl !== null ? Math.max(m, lvl) : m;
    }, 0);
    req('English Level', `ICAO Level ${job.reqEnglishLevel}`, 'Languages', maxLevel >= job.reqEnglishLevel, maxLevel > 0 ? `Level ${maxLevel}` : null);
  }
  if (job.reqMinMultiEngineHours != null) {
    const multi = totals?.multiEngineTime ?? 0;
    req('Multi-Engine', `${job.reqMinMultiEngineHours.toLocaleString()} hrs min`, 'Wrench', multi >= job.reqMinMultiEngineHours, `${Math.round(multi).toLocaleString()} hrs`);
  }
  if (job.reqMinTurbineHours != null) {
    const turb = totals?.turbineTime ?? 0;
    req('Turbine Time', `${job.reqMinTurbineHours.toLocaleString()} hrs min`, 'Wrench', turb >= job.reqMinTurbineHours, `${Math.round(turb).toLocaleString()} hrs`);
  }
  if (job.reqMinInstrumentHours != null) {
    const inst = totals?.instrumentTime ?? 0;
    req('Instrument', `${job.reqMinInstrumentHours.toLocaleString()} hrs min`, 'Wrench', inst >= job.reqMinInstrumentHours, `${Math.round(inst).toLocaleString()} hrs`);
  }
  if (job.reqMinCrossCountryHours != null) {
    const cc = totals?.crossCountryTime ?? 0;
    req('Cross-Country', `${job.reqMinCrossCountryHours.toLocaleString()} hrs min`, 'MapPin', cc >= job.reqMinCrossCountryHours, `${Math.round(cc).toLocaleString()} hrs`);
  }
...
export function matchStyle(score: number): { label: string; color: string } {
...
export function postedAgo(postedAt: string | null | undefined): string | null {
...
export function formatSalary(job: AnyRec, compact = false): string | null {
  ...
  if (compact) {
    ...
    if (salaryMin != null && salaryMax != null && salaryMin !== salaryMax) return `${currency} ${fmt(salaryMin)}–${fmt(salaryMax)}${period}`.trim();
