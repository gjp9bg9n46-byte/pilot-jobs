...
exports.importParse = async (req, res, next) => {
  try {
    ...
    // Enrich the header-based mapping for crew rosters: synthesize Date/Off/On from
    // combined Start/Finish datetimes ("12Mar24 0029"), split a Route column
    // ("CAI-DMM") into Departure/Arrival, and disambiguate an "AC" column
    // (registration vs aircraft type). May add synthetic columns → reassign.
    ({ headers, rawRows } = enrichColumns(headers, rawRows, mapping));
